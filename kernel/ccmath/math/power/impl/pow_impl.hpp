...
#include "../../../internal/support/floating_point_traits.hpp"
#include "../../../internal/support/fp/bit_mask_traits.hpp"
#include "../../../internal/support/fp/directional_rounding_utils.hpp"
#include "../../../internal/support/fp/fp_bits.hpp"
#include "../../../internal/support/unreachable.hpp"
#include "../../../internal/types/big_int.hpp"
#include "../../basic/abs.hpp"
#include "../../compare/isnan.hpp"
#include "../sqrt.hpp"
...
namespace ccm::internal::impl
{
	...
	constexpr std::enable_if_t<std::is_floating_point_v<T>, T> pow_impl_handle_special_cases(support::fp::FPBits<T> & x_bits, support::fp::FPBits<T> & y_bits,
																							 T & x, T & y) noexcept
	{
		if (x_bits.is_nan())
		{
	...
	(defined(__clang__) && defined(CCM_CONFIG_AGGRESSIVELY_OPTIMIZE)) || defined(__INTEL_LLVM_COMPILER)
			if (y == 0.0 && x_bits.is_signaling_nan()) { return 1.0; }
#endif
																							...
																							T & current_sign_of_result) noexcept
	{
		...
		current_sign_of_result = copy_sign.at(static_cast<std::size_t>(y_parity));
