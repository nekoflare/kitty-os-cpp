...
static driver_status_t vmware_svgaii_entry(pci_dev* pci_handle)
{
    ...
    vmware_write_reg(SVGA_REG_ID, 0x90000002);
    uint32_t v = vmware_read_reg(SVGA_REG_ID);
    if (v != 0x90000002)
    {
        kstd::printf("[VMW] VMWare doesn't support the newest spec. The spec: %lx\n", v);
        //unreachable();
...
)
{
    ...
    kstd::printf("%x %x %x", vmware_read_reg(SVGA_REG_WIDTH), vmware_read_reg(SVGA_REG_HEIGHT), vmware_read_reg(SVGA_REG_BPP));
