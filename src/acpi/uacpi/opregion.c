...
void uacpi_trace_region_error(uacpi_namespace_node *node, uacpi_char *message, uacpi_status ret)
{
    ...
    obj = uacpi_namespace_node_get_object_typed(node, UACPI_OBJECT_OPERATION_REGION_BIT);
...
static void trace_region_io(uacpi_field_unit *field, uacpi_address_space space, uacpi_u64 offset, uacpi_region_op op,
                            union uacpi_opregion_io_data data)
{
    ...
    case UACPI_ADDRESS_SPACE_FFIXEDHW:
        uacpi_trace("write-then-read from [%s] %s[0x%016" UACPI_PRIX64 "] = "
                    "<buffer of %zu bytes>\n",
                    path, uacpi_address_space_to_string(space), UACPI_FMT64(offset), data.buffer.length);
        break;
    ...
    case UACPI_ADDRESS_SPACE_GENERAL_PURPOSE_IO:
        uacpi_trace("%s [%s] %s pins[%u..%u] = 0x%" UACPI_PRIX64 "\n", type_str, path,
                    uacpi_address_space_to_string(space), field->pin_offset,
                    (field->pin_offset + field->bit_length) - 1, UACPI_FMT64(*data.integer));
        break;
    default:
        uacpi_trace("%s [%s] (%d bytes) %s[0x%016" UACPI_PRIX64 "] = 0x%" UACPI_PRIX64 "\n", type_str, path,
                    field->access_width_bytes, uacpi_address_space_to_string(space), UACPI_FMT64(offset),
...
{
    if (space == UACPI_ADDRESS_SPACE_SYSTEM_MEMORY || space == UACPI_ADDRESS_SPACE_SYSTEM_IO ||
...
static uacpi_status region_run_reg(uacpi_namespace_node *node, uacpi_u8 connection_code)
{
    ...
    ret = uacpi_namespace_node_resolve(node->parent, "_REG", UACPI_SHOULD_LOCK_NO, UACPI_MAY_SEARCH_ABOVE_PARENT_NO,
                                       UACPI_PERMANENT_ONLY_NO, &reg_node);
    if (uacpi_unlikely_error(ret))
        return ret;

    reg_obj = uacpi_namespace_node_get_object_typed(reg_node, UACPI_OBJECT_METHOD_BIT);
    ...
    ret = uacpi_execute_control_method(reg_node, reg_obj->method, &method_args, UACPI_NULL);
    if (uacpi_unlikely_error(ret))
...
out:
    if (region->state_flags & UACPI_OP_REGION_STATE_ATTACHED)
    {
        ...
        if (uacpi_unlikely_error(ret))
        {
            uacpi_trace_region_error(node, "error during handler detach for", ret);
        }
    }

    if ((region->state_flags & UACPI_OP_REGION_STATE_REG_EXECUTED) && unreg == UNREG_YES)
    {
...
static uacpi_iteration_decision do_install_or_uninstall_handler(uacpi_handle opaque, uacpi_namespace_node *node,
                                                                uacpi_u32 depth)
{
    ...
    if (object->type == UACPI_OBJECT_OPERATION_REGION)
    {
        ...
        else
        {
            ...
            {
                uacpi_trace_region_error(node, "handler mismatch for", UACPI_STATUS_INTERNAL_ERROR);
                return UACPI_ITERATION_DECISION_CONTINUE;
            }
...
static uacpi_status reg_or_unreg_all_opregions(uacpi_namespace_node *device_node, enum uacpi_address_space space,
                                               uacpi_u8 connection_code)
{
    uacpi_address_space_handlers *handlers;
    ...
    if (uacpi_unlikely(is_connect && find_handler(handlers, space) == UACPI_NULL))
        return UACPI_STATUS_NO_HANDLER;
    ...
    uacpi_namespace_do_for_each_child(device_node, do_run_reg, UACPI_NULL, UACPI_OBJECT_OPERATION_REGION_BIT,
    ...
    uacpi_trace("%sactivated all '%s' opregions controlled by '%.4s', "
                ...
                connection_code == ACPI_REG_CONNECT ? "" : "de", uacpi_address_space_to_string(space),
...
static uacpi_address_space_handlers *extract_handlers(uacpi_namespace_node *node)
{
    ...
    handlers_obj = uacpi_namespace_node_get_object_typed(node, UACPI_OBJECT_DEVICE_BIT | UACPI_OBJECT_THERMAL_ZONE_BIT |
                                                            ...
                                                            uacpi_u16 flags)
{
    ...
    uacpi_namespace_do_for_each_child(device_node, do_install_or_uninstall_handler, UACPI_NULL, UACPI_OBJECT_ANY_BIT,
                                      UACPI_MAX_DEPTH_ANY, UACPI_SHOULD_LOCK_NO, UACPI_PERMANENT_ONLY_YES, &iter_ctx);
...
uacpi_status uacpi_install_address_space_handler(uacpi_namespace_node *device_node, enum uacpi_address_space space,
                                                 uacpi_region_handler handler, uacpi_handle handler_context)
{
    return uacpi_install_address_space_handler_with_flags(device_node, space, handler, handler_context, 0);
}
...
uacpi_status uacpi_uninstall_address_space_handler(uacpi_namespace_node *device_node, enum uacpi_address_space space)
{
    ...
    uacpi_namespace_do_for_each_child(device_node, do_install_or_uninstall_handler, UACPI_NULL, UACPI_OBJECT_ANY_BIT,
                                      UACPI_MAX_DEPTH_ANY, UACPI_SHOULD_LOCK_NO, UACPI_PERMANENT_ONLY_NO, &iter_ctx);
...
{
    return !uacpi_is_buffer_access_address_space(space) && space != UACPI_ADDRESS_SPACE_GENERAL_PURPOSE_IO;
}

uacpi_status uacpi_dispatch_opregion_io(uacpi_field_unit *field, uacpi_u32 offset, uacpi_region_op op,
                                        union uacpi_opregion_io_data data)
{
    uacpi_status ret;
    uacpi_object *obj;
    ...
    obj = uacpi_namespace_node_get_object_typed(field->region, UACPI_OBJECT_OPERATION_REGION_BIT);
    if (uacpi_unlikely(obj == UACPI_NULL))
    {
        ret = UACPI_STATUS_INVALID_ARGUMENT;
        goto out;
    ...
    if (uacpi_unlikely(is_oob))
    {
        ...
        uacpi_error("out-of-bounds access to opregion %s[0x%" UACPI_PRIX64 "->"
                    ...
                    path, UACPI_FMT64(region->offset), UACPI_FMT64(region->offset + region->length),
    ...
    case UACPI_ADDRESS_SPACE_PCC: {
        ...
        {
            uacpi_memcpy_zerout(data.integer, cursor, sizeof(*data.integer), field->access_width_bytes);
            goto io_done;
        }
    ...
    case UACPI_ADDRESS_SPACE_GENERAL_PURPOSE_IO:
        ...
        ret = uacpi_object_get_string_or_buffer(field->connection, &handler_data.gpio.connection);
        if (uacpi_unlikely_error(ret))
            goto io_done;

        op = op == UACPI_REGION_OP_READ ? UACPI_REGION_OP_GPIO_READ : UACPI_REGION_OP_GPIO_WRITE;
        break;
    case UACPI_ADDRESS_SPACE_IPMI:
    ...
    case UACPI_ADDRESS_SPACE_GENERIC_SERIAL_BUS:
    case UACPI_ADDRESS_SPACE_SMBUS:
        ret = uacpi_object_get_string_or_buffer(field->connection, &handler_data.serial.connection);
        if (uacpi_unlikely_error(ret))
            goto io_done;
        ...
        op = op == UACPI_REGION_OP_READ ? UACPI_REGION_OP_SERIAL_READ : UACPI_REGION_OP_SERIAL_WRITE;
        break;
    default:
