...
{
    static const uacpi_char *pci_root_ids[] = {PCI_ROOT_PNP_ID, PCI_EXPRESS_ROOT_PNP_ID, UACPI_NULL};
    ...
    uacpi_trace_region_error(node, "unable to find PCI root controlling", UACPI_STATUS_NOT_FOUND);
    return node;
}
...
static uacpi_status pci_region_attach(uacpi_region_attach_data *data)
{
    ...
    if (uacpi_unlikely(device == UACPI_NULL))
    {
        ...
        uacpi_trace_region_error(node, "unable to find device responsible for", ret);
        return ret;
    }
...
static uacpi_status pci_region_do_rw(uacpi_region_op op, uacpi_region_rw_data *data)
{
    ...
    return op == UACPI_REGION_OP_READ ? uacpi_pci_read(dev, offset, width, &data->value)
...
static uacpi_status memory_region_do_rw(uacpi_region_op op, uacpi_region_rw_data *data)
{
    ...
    return op == UACPI_REGION_OP_READ ? uacpi_system_memory_read(ctx->virt, offset, data->byte_width, &data->value)
...
static uacpi_status table_data_region_do_rw(uacpi_region_op op, uacpi_region_rw_data *data)
{
    ...
    return op == UACPI_REGION_OP_READ ? uacpi_system_memory_read(addr, 0, data->byte_width, &data->value)
...
static uacpi_status io_region_do_rw(uacpi_region_op op, uacpi_region_rw_data *data)
{
    ...
    return op == UACPI_REGION_OP_READ ? uacpi_system_io_read(ctx->handle, offset, width, &data->value)
...
void uacpi_install_default_address_space_handlers(void)
{
    ...
    uacpi_install_address_space_handler_with_flags(root, UACPI_ADDRESS_SPACE_SYSTEM_MEMORY, handle_memory_region,
                                                   UACPI_NULL, UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);

    uacpi_install_address_space_handler_with_flags(root, UACPI_ADDRESS_SPACE_SYSTEM_IO, handle_io_region, UACPI_NULL,
                                                   UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);

    uacpi_install_address_space_handler_with_flags(root, UACPI_ADDRESS_SPACE_PCI_CONFIG, handle_pci_region, UACPI_NULL,
                                                   UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);

    uacpi_install_address_space_handler_with_flags(root, UACPI_ADDRESS_SPACE_TABLE_DATA, handle_table_data_region,
                                                   UACPI_NULL, UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);
}
