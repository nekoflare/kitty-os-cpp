...
static uacpi_status access_field_unit(uacpi_field_unit *field, uacpi_u32 offset, uacpi_region_op op,
                                      union uacpi_opregion_io_data data)
{
    ...
    case UACPI_FIELD_UNIT_KIND_BANK:
        ret = uacpi_write_field_unit(field->bank_selection, &field->bank_value, sizeof(field->bank_value), UACPI_NULL);
        break;
    case UACPI_FIELD_UNIT_KIND_NORMAL:
        break;
    case UACPI_FIELD_UNIT_KIND_INDEX:
        ret = uacpi_write_field_unit(field->index, &offset, sizeof(offset), UACPI_NULL);
        if (uacpi_unlikely_error(ret))
            goto out;
        ...
        case UACPI_REGION_OP_READ:
            ret = uacpi_read_field_unit(field->data, data.integer, field->access_width_bytes, UACPI_NULL);
            break;
        case UACPI_REGION_OP_WRITE:
            ret = uacpi_write_field_unit(field->data, data.integer, field->access_width_bytes, UACPI_NULL);
            break;
        default:
...
static uacpi_status handle_special_field(uacpi_field_unit *field, uacpi_data_view buf, uacpi_region_op op,
                                         uacpi_data_view *wtr_response, uacpi_bool *did_handle)
{
    uacpi_status ret = UACPI_STATUS_OK;
    uacpi_object *obj;
    ...
    *did_handle = UACPI_FALSE;
    ...
    {
        ret = UACPI_STATUS_INVALID_ARGUMENT;
        uacpi_trace_region_error(field->region, "attempted access to deleted", ret);
        goto out_handled;
    }
    ...
    case UACPI_ADDRESS_SPACE_GENERAL_PURPOSE_IO:
        ...
        {
            uacpi_memcpy_zerout(&in_out, buf.const_data, sizeof(in_out), buf.length);
        }
    ...
    case UACPI_ADDRESS_SPACE_PRM:
        if (uacpi_unlikely(op == UACPI_REGION_OP_READ))
        {
            ...
            uacpi_trace_region_error(field->region, "attempted to read from a write-only", ret);
            goto out_handled;
        }
        UACPI_FALLTHROUGH;
    case UACPI_ADDRESS_SPACE_FFIXEDHW:
...
do_wtr:
    ...
    uacpi_memcpy_zerout(wtr_buffer.data, buf.const_data, wtr_buffer.length, buf.length);
    ret = access_field_unit(field, field->byte_offset, op,
                            (union uacpi_opregion_io_data){
                                .buffer = wtr_buffer,
                            });
    if (uacpi_unlikely_error(ret))
...
static uacpi_status do_read_misaligned_field_unit(uacpi_field_unit *field, uacpi_u8 *dst, uacpi_size size)
{
    ...
    struct bit_span dst_span = {.data = dst, .index = 0, .length = size * 8};

    reads_to_do = UACPI_ALIGN_UP(field->bit_offset_within_first_byte + field->bit_length, width_access_bits, uacpi_u32);
    reads_to_do /= width_access_bits;

    while (reads_to_do-- > 0)
    {
        src_span.length = UACPI_MIN(bits_left, width_access_bits - src_span.index);

        ret = access_field_unit(field, byte_offset, UACPI_REGION_OP_READ, OPREGION_IO_U64(&out));
        if (uacpi_unlikely_error(ret))
            return ret;
...
uacpi_status uacpi_read_field_unit(uacpi_field_unit *field, void *dst, uacpi_size size, uacpi_data_view *wtr_response)
{
    ...
    ret = handle_special_field(field,
                               (uacpi_data_view){
                                   .data = dst,
                                   .length = size,
                               },
                               UACPI_REGION_OP_READ, wtr_response, &did_handle);
    if (did_handle)
        return ret;
     ...
     */
    if (field->bit_offset_within_first_byte == 0 && field_byte_length <= field->access_width_bytes)
    {
        uacpi_u64 out;

        ret = access_field_unit(field, field->byte_offset, UACPI_REGION_OP_READ, OPREGION_IO_U64(&out));
        if (uacpi_unlikely_error(ret))
            return ret;
...
static uacpi_status write_generic_field_unit(uacpi_field_unit *field, const void *src, uacpi_size size)
{
    ...
    struct bit_span src_span = {.const_data = src, .index = 0, .length = size * 8};
    struct bit_span dst_span = {
        .data = (uacpi_u8 *)&in,
        .index = field->bit_offset_within_first_byte,
    };
    ...
    {
        in = 0;
        dst_span.length = UACPI_MIN(width_access_bits - dst_span.index, bits_left);
        ...
        if (dst_span.index != 0 || dst_span.length < width_access_bits)
        {
            ...
            case UACPI_UPDATE_RULE_PRESERVE:
                ret = access_field_unit(field, byte_offset, UACPI_REGION_OP_READ, OPREGION_IO_U64(&in));
                if (uacpi_unlikely_error(ret))
                    return ret;
                break;
        ...
        ret = access_field_unit(field, byte_offset, UACPI_REGION_OP_WRITE, OPREGION_IO_U64(&in));
        if (uacpi_unlikely_error(ret))
            return ret;
...
uacpi_status uacpi_write_field_unit(uacpi_field_unit *field, const void *src, uacpi_size size,
                                    uacpi_data_view *wtr_response)
{
    ...
    ret = handle_special_field(field,
                               ...
                               UACPI_REGION_OP_WRITE, wtr_response, &did_handle);
    if (did_handle)
        return ret;
...
static uacpi_u8 gas_get_access_bit_width(const struct acpi_gas *gas)
{
    ...
    if (gas->register_bit_offset == 0 && UACPI_IS_POWER_OF_TWO(gas->register_bit_width, uacpi_u8) &&
    ...
    else
    {
        ...
        msb = uacpi_bit_scan_backward((gas->register_bit_offset + gas->register_bit_width) - 1);
        ...
        else
        {
             ...
             */
            while (!UACPI_IS_ALIGNED(gas->address, access_bit_width / 8, uacpi_u64))
                access_bit_width /= 2;
        }
    }

    return UACPI_MIN(access_bit_width, gas->address_space_id == UACPI_ADDRESS_SPACE_SYSTEM_IO ? 32 : 64);
}
...
uacpi_status uacpi_map_gas_noalloc(const struct acpi_gas *gas, uacpi_mapped_gas *out_mapped)
{
    ...
    { // IO, validated by gas_validate above
