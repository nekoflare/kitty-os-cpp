...
#define MMAP(addr, size, prot, flags)                                                                                  \
    ((dev_zero_fd < 0)                                                                                                 \
         ? (dev_zero_fd = open("/dev/zero", O_RDWR), mmap((addr), (size), (prot), (flags), dev_zero_fd, 0))            \
...
static void do_check_malloc_state(void)
{
    ...
    for (i = 1; i < NBINS; ++i)
    {
        ...
        for (p = last(b); p != b; p = p->bk)
        {
            ...
            if (i >= 2)
            {
                ...
                {
                    assert(p->bk == b || (CHUNK_SIZE_T)chunksize(p->bk) >= (CHUNK_SIZE_T)chunksize(p));
                }
            }
            /* chunk is followed by a legal chain of inuse chunks */
            for (q = next_chunk(p); (q != av->top && inuse(q) && (CHUNK_SIZE_T)(chunksize(q)) >= MINSIZE);
                 q = next_chunk(q))
    ...
    assert((CHUNK_SIZE_T)(av->sbrked_mem) <= (CHUNK_SIZE_T)(av->max_sbrked_mem));

    assert((CHUNK_SIZE_T)(av->mmapped_mem) <= (CHUNK_SIZE_T)(av->max_mmapped_mem));

    assert((CHUNK_SIZE_T)(av->max_total_mem) >= (CHUNK_SIZE_T)(av->mmapped_mem) + (CHUNK_SIZE_T)(av->sbrked_mem));
}
#endif
...
#endif
{
    ...
    */
    ...
    assert((old_top == initial_top(av) && old_size == 0) ||
           ((CHUNK_SIZE_T)(old_size) >= MINSIZE && prev_inuse(old_top)));
            ...
            */
                ...
                */
                ...
                if (front_misalign > 0)
                {
                    ...
                    /*
                      ...
                      prev_inuse of av->top (and any chunk created from its
                ...
                /*
                  ...
                  be able to merge with old_top space, so must add to 2nd
                  request.
                */
            ...
            if (snd_brk != (char *)(MORECORE_FAILURE))
            {
                ...
                /*
                  ...
                  double fencepost at old_top to prevent consolidation with
                  space we don't own. These fenceposts are artificial chunks
                  that are marked as inuse and are in any case too small to use.
    ...
    if (!chunk_is_mmapped(oldp))
    {
        ...
        else
        {
             ...
             */
                     ...
                     (CHUNK_SIZE_T)(newsize = oldsize + chunksize(next)) >= (CHUNK_SIZE_T)(nb))
            {
                newp = oldp;
                unlink(next, bck, fwd);
            }
    ...
    */
    ...
    { /* misaligned */
        ...
        */
        ...
        brk = (char *)mem2chunk((PTR_UINT)(((PTR_UINT)(m + alignment - 1)) & -((signed long)alignment)));
        ...
        assert(newsize >= nb && (((PTR_UINT)(chunk2mem(p))) % alignment) == 0);
    }
...
#if __STD_C
static Void_t **iALLOc(size_t n_elements, size_t *sizes, int opts, Void_t *chunks[])
#else
...
static void *sbrk(long size)
{
    ...
    assert((char *)g_last->top_reserved - g_last->reserve_size <= (char *)g_last->top_committed &&
           g_last->top_committed <= g_last->top_reserved && (unsigned)g_last->top_committed % g_pagesize == 0);
            ...
            if (to_reserve > 0)
            {
                ...
                if (remaining_commit_size > 0)
                {
                    ...
                    assert(0 < remaining_commit_size && remaining_commit_size % g_pagesize == 0);
                    {
                        /* Commit this */
                        void *base_committed =
                            VirtualAlloc(g_last->top_committed, remaining_commit_size, MEM_COMMIT, PAGE_READWRITE);
...
static int cpuinfo(int whole, CHUNK_SIZE_T *kernel, CHUNK_SIZE_T *user)
{
    if (whole)
    {
        ...
        int rc = GetProcessTimes(GetCurrentProcess(), (FILETIME *)&creation64, (FILETIME *)&exit64,
                                 (FILETIME *)&kernel64, (FILETIME *)&user64);
        if (!rc)
        {
            *kernel = 0;
            *user = 0;
    ...
    {
        __int64 creation64, exit64, kernel64, user64;
        int rc = GetThreadTimes(GetCurrentThread(), (FILETIME *)&creation64, (FILETIME *)&exit64, (FILETIME *)&kernel64,
                                (FILETIME *)&user64);
        if (!rc)
        {
            *kernel = 0;
