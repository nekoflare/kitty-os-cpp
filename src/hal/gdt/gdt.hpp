...
void load_gdt(void);
extern "C" void flush_gdt(gdtr *gdtr);

extern struct gdt_segment_descriptor gdt[];

#define GDT_ENTRY(_Base, _Limit, _Access, _Flags)                                                                      \
    {                                                                                                                  \
        .limit_one = static_cast<uint16_t>(_Limit & 0xFFFF), .base_zero = static_cast<uint16_t>(_Base & 0xFFFF),       \
        .base_two = static_cast<uint8_t>((_Base & 0xFF0000) >> 16), .access_byte = _Access,                            \
        .limit_two = static_cast<uint8_t>((_Limit & 0xF0000) >> 16), .flags = _Flags,                                  \
